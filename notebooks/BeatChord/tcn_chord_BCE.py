...
train_t_1hot = targets_to_one_hot(train_t)
valid_t_1hot = targets_to_one_hot(valid_t)
test_t_1hot = targets_to_one_hot(test_t)

print('example of 1-hot-encoded target shape:', train_t_1hot[0].shape)
...
# kernels
cnn_k_1_size = 3
cnn_k_2_size = (1, 2) # LAST_CNN_KERNEL_FREQUENCY_SIZE
cnn_padding = (1,0)
...
class TCNChordSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        transposed_target = np.transpose(np.asarray(target))
        return torch.from_numpy(sample).unsqueeze_(0), torch.from_numpy(transposed_target)
...
def run_training():
    ...
    train_loader = torch.utils.data.DataLoader(TCNChordSet(train_f, train_t_1hot, args.context, args.hop_size),
                                               batch_size=args.batch_size, shuffle=True, **kwargs)
    valid_loader = torch.utils.data.DataLoader(TCNChordSet(valid_f, valid_t_1hot, args.context, args.hop_size),
                                               batch_size=args.batch_size, shuffle=False, **kwargs)
    test_loader = torch.utils.data.DataLoader(TCNChordSet(test_f, test_t_1hot, args.context, args.hop_size),
                                              batch_size=args.batch_size, shuffle=False, **kwargs)
